...
<body class="bg-smart-bg text-white min-h-screen flex items-center justify-center p-4 selection:bg-smart-red selection:text-white">
  ...
  <div class="w-full max-w-md space-y-6">
    ...
    <div class="text-center space-y-2">
      ...
      <h1 class="text-2xl font-extrabold tracking-tight uppercase">RECUPERAR CONTRASEÑA</h1>
      <p class="text-xs text-smart-text-muted uppercase">INGRESA TU CURP Y CORREO ELECTRÓNICO PARA RECIBIR LAS INSTRUCCIONES</p>
    </div>
    ...
    <div class="bg-smart-card border border-smart-border rounded-2xl p-6 shadow-2xl space-y-5">
      <form action="<?= base_url('login/send-reset-link') ?>" method="POST" class="space-y-4">
        ...
        <div class="space-y-1.5">
          <label for="email" class="text-xs font-bold uppercase tracking-wider text-smart-text-muted">CORREO ELECTRÓNICO</label>
          <div class="relative">
            <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-smart-text-muted text-lg">mail</span>
            <input 
              type="email" 
              id="email" 
              name="email" 
...
<div class="flex justify-center pt-1">
  <a href="<?= base_url('/login') ?>" class="inline-flex items-center gap-2 text-xs font-semibold text-smart-text-muted hover:text-[#c03c3c] transition-colors uppercase">
    <span class="material-symbols-outlined text-base">arrow_back</span>
    <span>REGRESAR AL INICIO DE SESION</span>
  </a>
</div>
     <!-- BOTÓN REGRESAR AL INICIO CENTRADO -->
<div class="flex justify-center pt-1">
